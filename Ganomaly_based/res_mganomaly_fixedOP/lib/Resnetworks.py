...
class Encoder_RES_GANomaly(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, add_final_conv=True, extra_res: int = 0):
        ...
        for i in range(num_pyramid_stages):
            ...
            main.add_module(f'pyramid-{i}-drop-{out_feat}', nn.Dropout2d(p=0.15))
...
class BranchClassifier(nn.Module):
    """
    A branch classifier that pools the discriminator’s feature map
    down to (B, C_feat, 1, 1), then applies a 1×1 conv to produce
    exactly n_branches logits per example.
    """
    def __init__(self, feat_channels: int, n_branches: int):
        super().__init__()
        # 1) pool spatial dims to 1x1
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.drop = nn.Dropout(p=0.3)
        # 2) 1×1 conv → exactly n_branches channels
        self.conv = nn.Conv2d(feat_channels, n_branches,
                              kernel_size=1, stride=1, bias=False)
        # apply your existing weight initialization
        self.apply(weights_init)

    def forward(self, feats: torch.Tensor) -> torch.Tensor:
        """
        feats: (B, C_feat, H, W)
        returns: (B, n_branches)
        """
        x = self.pool(feats)
        x = self.drop(x)# → (B, C_feat, 1, 1)
        x = self.conv(x)               # → (B, n_branches, 1, 1)
        return x.view(x.size(0), -1)   # → (B, n_branches)
